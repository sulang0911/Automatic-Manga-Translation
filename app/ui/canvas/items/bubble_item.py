...
class BubbleItemSignals(QObject):
    ...
    geometry_start = pyqtSignal(dict)
    ...
    force_erase_toggled = pyqtSignal(dict)
...
class BubbleItem(QGraphicsRectItem):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            self.signals.geometry_start.emit(self.block_data)
    ...
    def contextMenuEvent(self, event):
        ...
        act_force_erase = None
        if self.block_data.get("type") == "onomatopoeia":
            is_forced = bool(self.block_data.get("force_erase", False))
            act_force_erase = menu.addAction("☑ 取消强制抹除并翻译" if is_forced else "☐ 强制抹除并翻译")
            menu.addSeparator()
        ...
        if act_force_erase and action == act_force_erase:
            self.block_data["force_erase"] = not is_forced
            self.signals.force_erase_toggled.emit(self.block_data)
        elif action == act_merge_prev:
