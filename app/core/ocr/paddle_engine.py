...
class PaddleOCREngine(BaseOCREngine):
    ...
    def _get_ocr(self):
        if self._ocr is None:
            # Set thread limits to prevent CPU freeze on Windows
            ...
            if self._is_old_gpu and has_paddle_cuda:
                logger.warning("Pascal/Maxwell GPU detected: setting ocr_version='PP-OCRv3' for stability.")
        ...
        if results and len(results) > 0 and results[0]:
            ...
            if isinstance(results[0], dict):
                ...
                for i in range(len(rec_texts)):
                    ...
                    if not text or conf < 0.20 or poly is None:
                        continue
